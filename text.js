(Scratch => {
  ...
  class ScratchText {
    ...
    capitalize_case_block({A, B}) {
      ...
      const text = cast.toString(A).split(split).map(word => word === undefined ? split : word[0].toUpperCase() + (word.substring(1) === undefined ? '' : word.substring(1).toLowerCase()) + split).join('');
